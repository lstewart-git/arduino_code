/*
  Calculate pilot dispatch reliability given a certain amount of sick days
  using Monmte Carlo numerical techniques
  this is for the case 3 pilots, 3 aircraft, no spares
 */
...
int sickCount = 0;
long days = 40000;     //   length of the model run, in days
int failDay = 0;     //   amount of days with an unacceptable absence rate
int allowedSickies = 0;   // number of absent employees allowed on any day
float perCent = 1.0;
...
int dayFcn (int sickDaysYR) {
 ...
 // edit out for no reserve pilot:     long NR = random(1,364);
  
  
    if (LS <=sickDaysYR){sick = sick + 1;}
    if (GF <=sickDaysYR){sick = sick + 1;}
    if (RS <=sickDaysYR){sick = sick + 1;}
  // edit out for no reserve pilot:       if (NR <=sickDaysYR){sick = sick + 1;}
